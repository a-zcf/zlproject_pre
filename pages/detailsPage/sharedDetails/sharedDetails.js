...
Page({
  ...
  getHouseDetail({id}) {
    ...
    request({
      ...
      realSuccess: function (data) {
        ...
        let { baseServicer, enterpriseServicer, introductionDetail, lat, lng,title} = sharedDetail
        if (baseServicer != null){
          WxParse.wxParse('baseservicer', 'html', baseServicer, that, 0);
        }else{
        }
        if (enterpriseServicer != ''){
          WxParse.wxParse('entservicer', 'html', enterpriseServicer, that, 0);
        }else{
        }
        // if (describes != ''){
  ...
  onShareAppMessage: function (e) {
    return {
      title: '分享',
      path: 'pages/detailsPage/sharedDetails/sharedDetails?id=' + this.data.posId,
